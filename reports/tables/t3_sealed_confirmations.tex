% Auto-generated from step5_confirm_v3a (9e6) + step5_confirm_v1b (13e6).
\begin{tabular}{lllrrrc}
\toprule
config & confirmation block & seed & vs fixed & vs adaptive & $p$ (adaptive) & valid \\
\midrule
faster-learning (v3a) & 9{,}000{,}000 & 5 & +0.0221 & +0.0222 & 0.749 & yes \\
 &  & 6 & -0.0104 & -0.0103 & 0.813 & yes \\
 &  & 7 & +0.0028 & +0.0028 & 0.804 & yes \\
 &  & 8 & -0.0080 & -0.0079 & 0.936 & yes \\
 &  & 9 & -0.0184 & -0.0184 & 0.758 & yes \\
\cmidrule(lr){2-7}
 & \multicolumn{6}{l}{pooled -0.0023 bps; $p$ = 0.379; 3/5 cheaper; \textbf{PASS = FALSE}} \\
\midrule
bigger-network (v1b) & 13{,}000{,}000 & 10 & -0.0010 & -0.0019 & 0.650 & yes \\
 &  & 11 & +0.0042 & +0.0033 & 0.953 & yes \\
 &  & 12 & -0.0297 & -0.0306 & 0.294 & yes \\
 &  & 13 & +0.0153 & +0.0144 & 0.491 & yes \\
 &  & 14 & +0.0047 & +0.0038 & 0.961 & yes \\
\cmidrule(lr){2-7}
 & \multicolumn{6}{l}{pooled -0.0022 bps; $p$ = 0.394; 2/5 cheaper; \textbf{PASS = FALSE}} \\
\bottomrule
\end{tabular}